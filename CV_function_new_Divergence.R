...
CVNB_D = function(M, K = 10, start_alpha = 10, n_mutatypes = 96, n_cv_sets = 10, n_updates = 5, CVentries_percent = 0.01){
  ...
  for (k in 2:max(K_range)){
    for (i in 1:n_cv_sets){
      alpha <- start_alpha
      M_CV_NB <- M
      M_CV_NB[CV_idx[[i]]] <- 0.001 #Replace approximately 1% of cells in matrix by 0.
      for (s in 1:n_updates) {
        ...
        D_alpha_CV_NB <- rbind(D_alpha_CV_NB, c(k, alpha, i, s, D_alpha, BIC, MSE))
  ...
  end_time <- Sys.time()
  colnames(D_alpha_CV_NB) <- c("K", "alpha", "CV_set", "n_update", "D_alpha", "BIC", "MSE")
